...
type
  ...
  public
    { public declarations }
     ...
     procedure RemplirAdjacent(aX,aY, color: integer);
...
//                TentativeSolide.SetBoolean(X-1,Y-1,true);
        ...
        if ((X<Form3.Width-130) and (Y<Form3.Height-10) and (X>10) and (Y>10))// and (Canvas.Pixels[X,Y] = clBtnFace))
        then begin
 //           Form5.Close;
...
procedure RemplirAdjacent(aX,aY, color: integer);
begin
    Form3.Canvas.Pixels[aX,aY] := clBlack;
    if (Form3.Canvas.Pixels[aX-1,aY] = color) and (aX>11)
    then RemplirAdjacent(aX-1,aY, color);
    if (Form3.Canvas.Pixels[aX,aY-1] = color) and (aY>11)
    then RemplirAdjacent(aX,aY-1, color);
    if (Form3.Canvas.Pixels[aX+1,aY] = color) and (aX<Form3.Width-131)
    then RemplirAdjacent(aX+1,aY, color);
    if (Form3.Canvas.Pixels[aX,aY+1] = color) and (aY<Form3.Height-11)
    then RemplirAdjacent(aX,aY+1,color);
end;
